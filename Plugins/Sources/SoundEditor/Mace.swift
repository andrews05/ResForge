...
/// Decoder for Macintosh Audio Compression/Expansion.
struct Mace {
    static func decode(data: Data, packetSize: Int, numChannels: Int) -> [Int16] {
        var output: [Int16] = []
        for i in 0..<numChannels {
            var chd = ChannelData(index: 0, factor: 0, prev2: 0, previous: 0, level: 0)

            for j in 0..<(data.count/(numChannels * packetSize)) {
                for k in 0..<packetSize {
                    let pkt = data[data.startIndex + i * packetSize + j * numChannels * packetSize + k]
                    let val = [
                        [pkt >> 5, (pkt >> 3) & 3, pkt & 7 ],
                        [pkt & 7 , (pkt >> 3) & 3, pkt >> 5],
                    ]
                    ...
                    for l in 0..<3 {
                        if packetSize == 2 {
                            chd.chomp3(output: &output, val: val[1][l], tabIdx: l)
                        } else {
                            chd.chomp6(output: &output, val: val[0][l], tabIdx: l)
                        }
                    }
                }
            }
        ...
        let numSamples = output.count / numChannels
        var interleaved: [Int16] = []
        for i in 0..<numSamples {
            for j in 0..<numChannels {
                interleaved.append(output[j * numSamples + i])
            }
        }
        return interleaved
    }
    ...
    struct ChannelData {
        static let tab1: [Int16] = [-13, 8, 76, 222, 222, 76, 8, -13]
        static let tab3: [Int16] = [-18, 140, 140, -18]
        static let tab2: [[Int16]] = [
        ...
        static let tabs = [
            Tab(tab1: tab1, tab2: tab2, stride: 4),
            Tab(tab1: tab3, tab2: tab4, stride: 2),
            Tab(tab1: tab1, tab2: tab2, stride: 4),
        ]
        ...
        func qt8Sto16S(_ x: Int16) -> Int16 {
            let hi = x >> 8
            return (hi << 8) | hi
        }

        mutating func readTable(val: UInt8, tabIdx: Int) -> Int16 {
            let tab = Self.tabs[tabIdx]
            let offset = Int((index & 0x7F0) >> 4)
            let current = if val < tab.stride {
                tab.tab2[offset][Int(val)]
            } else {
                -1 - tab.tab2[offset][2 * tab.stride - Int(val) - 1]
            }

            index += tab.tab1[Int(val)] - (index >> 5)
            if index < 0 {
                index = 0
            }
        ...
        mutating func chomp3(output: inout [Int16], val: UInt8, tabIdx: Int) {
            var current = readTable(val: val, tabIdx: tabIdx)

            current = Int16(clamping: Int(current) + Int(level))

            level = current - (current >> 3)
            output.append(self.qt8Sto16S(current))
        }

        mutating func chomp6(output: inout [Int16], val: UInt8, tabIdx: Int) {
            var current = readTable(val: val, tabIdx: tabIdx)

            if (previous ^ current) >= 0 {
                factor = Int16(clamping: Int(factor) + 506)
            } else {
                factor = Int16(clamping: Int(factor) - 314)
            }

            current = Int16(clamping: Int(current) + Int(level))

            level = Int16((Int(current) * Int(factor)) >> 15)
            current >>= 1

            output.append(self.qt8Sto16S(previous + prev2 - ((prev2-current) >> 2)))
            output.append(self.qt8Sto16S(previous + current + ((prev2-current) >> 2)))
            prev2 = previous
            previous = current
        }
    }
