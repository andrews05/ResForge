...
/// This view must be an immediate subview of a ``MenuDocumentView``.
class MenuItemView : NSView {
    ...
    var type: MenuItem.MenuItemType {
        didSet {
            reloadImage()
        }
    }
    ...
    init(frame frameRect: NSRect, title: String, type: MenuItem.MenuItemType, enabled: Bool, resourceID: Int, manager: RFEditorManager) {
        self.title = title
        self.type = type
    ...
    private func trackKnob(_ index: Int, for startEvent: NSEvent) {
        ...
        while keepTracking {
            ...
            if let currEvent = currEvent {
                ...
                case .leftMouseDragged:
                    ...
                    var box = frame
                    if MenuItemView.rightEdgeIndices.contains(index) {
                        box.size.width += distance.width
                    } else if MenuItemView.leftEdgeIndices.contains(index) {
                        box.size.width -= distance.width
                        box.origin.x += distance.width
                    }
                    if MenuItemView.bottomEdgeIndices.contains(index) {
                        box.size.height += distance.height
                    } else if MenuItemView.topEdgeIndices.contains(index) {
                        box.size.height -= distance.height
        ...
        if didChange {
            ...
            NotificationCenter.default.post(name: MenuDocumentView.itemFrameDidChangeNotification, object: superview)
        }
    }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        if event.modifierFlags.contains(.shift) { // Multi-selection.
            NotificationCenter.default.post(name: MenuDocumentView.selectionWillChangeNotification, object: superview)
            selected = !selected
            NotificationCenter.default.post(name: MenuDocumentView.selectionDidChangeNotification, object: superview)
            ...
            if (event.clickCount % 2) == 0 {
                NotificationCenter.default.post(name: MenuDocumentView.itemDoubleClickedNotification, object: superview, userInfo: [MenuDocumentView.doubleClickedItemView: self])
            } else {
                trackDrag(for: event)
            }
